...
def parse_args() -> argparse.Namespace:
    ...
    default_input = script_dir / "data" / "raw_combined_unbalanced_dataset.csv"
    default_output_dir = script_dir / "plots"
    parser = argparse.ArgumentParser(
        description="Run EDA for unified raw (not deduplicated, not balanced) sentiment dataset."
    )
    parser.add_argument(
        "--input",
        type=Path,
        default=default_input,
        help="Path to unified raw dataset CSV.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
...
def annotate_bars(ax: plt.Axes) -> None:
    ...
    for patch in ax.patches:
        ...
        y = patch.get_height()
        ax.text(x, y + offset, f"{int(y)}", ha="center", va="bottom", fontsize=10)
...
def main() -> None:
    ...
    sns.barplot(x=class_counts.index, y=class_counts.values, color="#5dade2", ax=ax)
    annotate_bars(ax)
    ax.set_title("Class Distribution (Raw Unified Dataset)")
    ax.set_xlabel("Sentiment label (0=negative, 1=neutral, 2=positive)")
    ...
    sns.barplot(x=source_counts.index.astype(str), y=source_counts.values, color="#58d68d", ax=ax)
    annotate_bars(ax)
    ax.set_title("Source Distribution (Raw Unified Dataset)")
    ...
    sns.histplot(words_for_hist, bins=50, kde=True, color="#16a085", ax=ax)
    ax.set_title("Text Length Distribution in Words (up to 99th percentile)")
    ...
    sns.boxplot(data=plot_df, x="label", y="word_len", order=[0, 1, 2], color="#aed6f1", ax=ax)
    ...
    sns.barplot(x=class_counts.index, y=class_counts.values, color="#5dade2", ax=axes[0, 0])
    annotate_bars(axes[0, 0])
    axes[0, 0].set_title("Class Distribution (Raw Unified Dataset)")
    axes[0, 0].set_xlabel("Sentiment label (0=negative, 1=neutral, 2=positive)")
    axes[0, 0].set_ylabel("Number of records")

    sns.barplot(x=source_counts.index.astype(str), y=source_counts.values, color="#58d68d", ax=axes[0, 1])
    annotate_bars(axes[0, 1])
    axes[0, 1].set_title("Source Distribution (Raw Unified Dataset)")
    axes[0, 1].set_xlabel("Source")
    axes[0, 1].set_ylabel("Number of records")
    axes[0, 1].tick_params(axis="x", rotation=20)

    sns.histplot(words_for_hist, bins=50, kde=True, color="#16a085", ax=axes[1, 0])
    axes[1, 0].set_title("Text Length Distribution in Words (up to 99th percentile)")
    axes[1, 0].set_xlabel("Text length (words)")
    axes[1, 0].set_ylabel("Frequency")
    axes[1, 0].set_xlim(0, p99)

    sns.boxplot(data=plot_df, x="label", y="word_len", order=[0, 1, 2], color="#aed6f1", ax=axes[1, 1])
    axes[1, 1].set_title("Text Length by Sentiment Class")
    axes[1, 1].set_xlabel("Sentiment label (0=negative, 1=neutral, 2=positive)")
    axes[1, 1].set_ylabel("Text length (words)")

    fig.suptitle("EDA Summary: Raw Unified Sentiment Dataset", fontsize=18)
    fig.tight_layout(rect=[0, 0, 1, 0.97])
    fig.savefig(output_dir / "00_eda_summary_4in1.png", dpi=300, bbox_inches="tight")
    plt.close(fig)

    print("\nPlots saved to:")
    for name in [
        "00_eda_summary_4in1.png",
